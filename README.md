...
## BSP-D3, a.k.a. BSP-D3 无线手柄
* USB线接PC：不需要按开机键直接用
* 红米9A，无线：按住方块，然后按手柄开机键
* 配套的蓝牙接收器，无线：按住L1，然后按手柄开机键
* 红米13C 5G，无线：按住R1，然后按手柄开机键，然后输入0000（当作Xbox Wireless Controller手柄用）
```
我买的杂牌安卓伸缩手柄到手了（68元），果然如我所料的，完全没问题——不会水深，
杂牌都能用，如果硬说不好，可能就是没有十字键而是方向按钮需要用摇杆代替。
我买的是Q17手柄，实际盒子上印的是BSP-D3，接电脑显示是XBOX360手柄。
按住方块开机可以配对安卓手机的蓝牙（当然它有很多种方式，要自己试），
蓝牙配对完之后自己在模拟器设置按键映射即可。我甚至试过蓝牙配对复古旗GPi Case 2W也可以，
只不过如果用来玩复古旗，那么这个手柄相当于第二手柄了，有些游戏的操作有时还是需要第一手柄来完成，
但控制单人游戏或者系统菜单是没问题的。之所以我那么自信，是因为我以前试过树莓派的杂牌手柄，
杂牌麦克风和杂牌摄像头这些廉价外设都没问题，甚至质量比贵的还耐用

我尝试用红米9A+安卓伸缩手柄（按住方块键再按开关键执行蓝牙配对）

用线连PC也可以用（需先长按手柄开机键，关闭手柄电源，然后再接USB线）

安卓拉伸手柄（BSP-D3）连PC蓝牙接收器（需配套买）而非手机蓝牙之效果
（按住L1按手柄开机键直接配对，不需要确认），
以解决串流和蓝牙冲突导致声音延迟之问题，效果如下，还不错，
虽然还是会有一点声音卡，但玩galgame手机串流一般不会卡 ​​​

红米13C 5G版需要按住R1，然后按开机键配对，然后输入0000
```
